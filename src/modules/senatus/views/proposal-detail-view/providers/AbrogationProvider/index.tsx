...
import { APIAbrogationEntity, fetchAbrogation } from 'modules/senatus/api';
import { useWallet } from 'wallets/wallet';

import { useDAO } from '../../../../components/dao-provider';
import { AbrogationProposalReceipt } from '../../../../contracts/daoSenatus';
...
const AbrogationProvider: React.FC = props => {
  ...
  React.useEffect(() => {
    ...
    fetchAbrogation(proposal.proposalId)
      ...
      .catch((status: number) => {
        ...
        history.push(`/senatus/proposals/${proposal.proposalId}`);
      });
  }, [proposal, version]);
  ...
  React.useEffect(() => {
    ...
    daoCtx.daoSenatus.actions.getAbrogationProposalReceipt(proposalId).then(receipt => {
      setState({
        receipt,
      });
    });
  ...
  function abrogationProposalCancelVote(gasPrice: number): Promise<void> {
    return proposal?.proposalId
      ? daoCtx.daoSenatus.actions.abrogationProposalCancelVote(proposal?.proposalId, gasPrice).then(() => reload())
      : Promise.reject();
  }
