...
import DAO_GOVERNANCE_ABI from './daoSenatus.json';
...
export type DAOSenatusContractData = {
  contract: Web3Contract;
  isActive?: boolean;
};

const InitialState: DAOSenatusContractData = {
  contract: Contract,
  isActive: undefined,
};

export type DAOSenatusContract = DAOSenatusContractData & {
  reload(): void;
  actions: {
...
export function useDAOSenatusContract(): DAOSenatusContract {
  const wallet = useWallet();

  const [state, setState] = useMergeState<DAOSenatusContractData>(InitialState);
